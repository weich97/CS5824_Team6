...
:--------------------------:|:-------------------------:
<img src="Screenshots/LDC.png" width="200" height="200">  |  <img src="Screenshots/2Dstep.png" width="200" height="200">
...
### Velocity Contours
The following figures show a training data sample and a prediction data sample, respectively. The velocity field contour of the training data sample is very smooth every well except at the singularity location (the center). However, this training data sample cannot be compared directly to the prediction data sample as they are not from the same distribution. We are not sure of why the paper uses such a way to present the results. If we only look at the velocity countour and streamlines, the GANs model generate a reasonable flow field for such problem, but there is no way to know what parameters these results for (we do not know the strength, the exact direction of uniform flow, etc.).
...
We think a better way to compare whether the results are good or not is to compare the true data with the false data generated by GANs from a same distribution (using the same Gaussian noise vector). We did it in our project.

### Velocity Divergence History
The following figure shows the effect of the tuning factor lambda over the convergence. A moderate number of lambda is preferred as it makes the convergence faster and better. Here using lambda=0 means that a standard GANs is used (no constraints enforced).
...
## Contours Plots
Attached countours are not given in the paper but we think it would be meaninful to present. After we finish training, we get the trained model. We can then use the trained model to validate a sample in the training dataset. If the prediction is very close to the original true sample, we would think the model is good enough, otherwise the trained model cannot reproduce the original CFD results. In this process, we want to investigate the effect of removing different numbers of pixel points near the sigularity, as this flow problem has a singularity at the center of the domain shown.

First, we found that the GANs model CANNOT reproduce the original data, although the velocity and pressure field looks reasonable. The solutions generated by the GANs model are close to physical solutions because the mean divergence is very close to 0. However, under what conditions (flow mass rate, velocity direction) the solutions are generated are unclear, becasue the velocity directions on the training sample and the prediction sample are different. This is not discussed at all in Ref [1] out of some reasons.

Seceond, if removing more data points near the sinularity, then the contour plot becomes quite different, possibly due to the slow convergence. However, an more important reason may be that the GANs cannot deal with data with large gradients very well. The authors in Ref [1] did not mention this at all. We guess the large gradient may be a difficulty for the GANs model or the physics-informed GANs model. 
...
Also, if we leave more points near the singularity, then the case cannot be converged well. The final mean divergence is more than 100 and cannot be driven down further. This indicates that the physcis-informed GANs cannot be used to train problems with large gradients, which is consistent with what we found in the countour plots section. This is also not specified in Ref [1]
