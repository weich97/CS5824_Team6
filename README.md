...
## Results
The following figures show a training data sample and a prediction data sample, respectively. However, this training data sample cannot be compared directly to the prediction data sample as they are not from the same distribution. We are not sure of why the paper uses such a way to present the results. We think a better way to compare whether the results are good or not is to compare the true data with the false data generated by GANs from a same distribution (using the same Gaussian noise vector). We did it in our project.

![image](Screenshots/Training_sample.png){A training sample in Ref [1]}
![image](Screenshots/Prediction_sample.png){A prediction sample in Ref [1]}
